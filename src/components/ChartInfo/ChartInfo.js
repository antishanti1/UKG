...
function calculatePsychSafetyScore() {
  const results = apiResponse.body.results;
  ...
  results.forEach(result => {
    ...
    // Sum the positive emotions
    totalPositive += emotions.confidence;
    totalPositive += emotions.joy;
    totalPositive += emotions.satisfaction;
    totalPositive += emotions.compassion;
    totalPositive += emotions.enthusiasm;

    // Sum the negative emotions
    totalNegative += emotions.anger;
    totalNegative += emotions.frustration;
    totalNegative += emotions.fear;
    totalNegative += emotions.confusion;
    totalNegative += emotions.disappointment;
  });

  // Calculate the average of the positive and negative emotion values
  const avgPositive = totalPositive / numberOfResults;
  const avgNegative = totalNegative / numberOfResults;

  // Calculate the psychological safety score
  const psychSafetyScore = (avgPositive - avgNegative /100) / 2;

  return psychSafetyScore;
};

const psychSafetyScore = calculatePsychSafetyScore(apiResponse);
console.log("Psychological Safety Score:", psychSafetyScore);

export default calculatePsychSafetyScore;